...
def multiprocess(function, input_list, args=None):
    ...
    for idx in range(num_batch):
        print(input_list[idx * batch_size:(idx + 1) * batch_size])
        p = pool.apply_async(function, args=(input_list[idx * batch_size:(idx + 1) * batch_size], args))
        results.append(p.get())

    return results

def exec_term_match(ts_files, args):
    # 17, 41
    PROJ_DIR = '/home/data/ldrs_analytics'
    check_date = "20230925_tm6"
    DISPLAY = False
    if DISPLAY:
        TS_folderpath = f"{PROJ_DIR}/data/docparse_csv/TS"
    else:
        # TS_folderpath = f"{PROJ_DIR}/data/updated_annotated_ts/bfilled"
    ...
    for f in ts_files:
        ...
        processed_titles = list([re.sub(r"([1-9]\d*\.?\d*)|(0\.\d*[1-9])", "", title).strip() for title in agr_para_titles])
...
def main():
    ...
    results = multiprocess(exec_term_match, ts_files)
