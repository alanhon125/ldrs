...
def get_identifier(row: pd.DataFrame) -> str:
    ...
    '''
    if row['section'] and row['sub_section']:
        isDefSection = re.match('Definition|Interpretation',str(row['section']), re.IGNORECASE) or re.match('Definition|Interpretation',str(row['sub_section']), re.IGNORECASE) or re.match('"(.*)" means.*', str(row['text']))
    elif row['section']:
        isDefSection = re.match('Definition|Interpretation',str(row['section']), re.IGNORECASE) or re.match('"(.*)" means.*', str(row['text']))
    else:
        isDefSection = re.match('"(.*)" means.*', str(row['text']))
    ...
    if isDefSection and list_id:
        if schedule_id or (sec_id != '0' and str(definition) not in ['nan', '', 'None']) or (identifier.strip() and identifier[-1].isdigit() and list_id[0].isdigit()):  # or identifier endwith digit and list id startwith digit
            identifier += '_' + str(list_id)
        elif sec_id != '0' and str(definition) in ['nan', '', 'None']:
            identifier += str(list_id)
    elif not isDefSection and list_id:
        splits = identifier.split('-')
        if len(splits) == 1:
            if splits[0].strip() and splits[0][-1].isdigit() and list_id[0].isdigit():
                identifier = splits[0] + '_' + str(list_id)
            else:
                identifier = splits[0] + str(list_id)
        else:
            if splits[0].strip() and splits[0]=='Parties':
                identifier = splits[0] + '-' + '-'.join(splits[1:])
            elif splits[0].strip() and splits[0][-1].isdigit() and list_id[0].isdigit():
                identifier = splits[0] + '_' + str(list_id) + '-' + '-'.join(splits[1:])
            else:
                identifier = splits[0] + str(list_id) + '-' + '-'.join(splits[1:])
...
def docparse2table(input_tuple: Iterable[Union[str, dict], str]) -> dict:
    ...
    df.loc[df['text_element'].str.contains('table'), 'text'] = df[df['text_element'].str.contains('table')]['text'].map(lambda x: json.dumps(x) if isinstance(x,(dict, list)) else x)
    ...
    if doc_type == 'FA':
        ...
        stringHasDef = df.text.str.contains('"(.*)" means.*',na=False)
        # replace " XXX " to "XXX" at any position in text
        df.loc[secHasDef | subsecHasDef | stringHasDef, 'text'] = df[secHasDef | subsecHasDef| stringHasDef]['text'].map(lambda x: re.sub(r'^" ', '"', re.sub(r'\. " ', '. "', re.sub(r'(?<!\.) " ', '"', x) if x else x) if x else x) if x else x)
        df.loc[secHasDef | subsecHasDef| stringHasDef, 'text'] = df[secHasDef | subsecHasDef| stringHasDef]['text'].map(lambda x: phrase_tokenize(x, nlp, delimiter_pattern=PARA_SPLIT_PATTERN) if x else x)
        df = df.explode('text')
        df.loc[secHasDef | subsecHasDef| stringHasDef, 'definition'] = df[secHasDef | subsecHasDef| stringHasDef]['text'].map(lambda x: extract_definition(x)[0])
        df.loc[(secHasDef | subsecHasDef| stringHasDef) & ~df.section_id.isnull() & ~df.sub_section_id.isnull(), 'definition'] = df[(secHasDef | subsecHasDef| stringHasDef) & ~df.section_id.isnull() & ~df.sub_section_id.isnull()].groupby(['section_id', 'sub_section_id'])['definition'].transform(lambda x: x.ffill())
