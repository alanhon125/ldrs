...
def float2percent(string):
    ...
    if isinstance(string, str):
        s = ast.literal_eval(string.strip())
    else:
        s = string
...
class RegenerateXlsx(object):
    ...
    def generate(self):
        ...
        #     self.df[['identifier_list_old', 'match_term_list_old', 'similarity_list_old']] = self.df[
        #     ['identifier_list_old', 'match_term_list_old', 'similarity_list_old']].map(lambda x: x.replace("('","['").replace("')","']") if x else x)
        #     self.df[['identifier_list_old', 'match_term_list_old', 'similarity_list_old']] = self.df[
        #     ['identifier_list_old', 'match_term_list_old', 'similarity_list_old']].map(
        #     lambda x: tuple(ast.literal_eval(x)) if isinstance(x, str) and (x.startswith('[') or x.startswith('(')) and x != 'None' else x)
        #     self.df['similarity_list_old'] = self.df['similarity_list_old'].map(lambda x: float2percent(x) if x else x)
        ...
        if not self.with_annot:
            ...
            # ,'TS_text_old','merge_reason', ,'New Judgement','New Matched Count','New Expected Count','match_term_list_old','identifier_list_old','similarity_list_old','Old Judgement','Old Matched Count','Old Expected Count','identical_identifier_count'
            self.df = self.df[['index','text_block_id','page_id','phrase_id','text_element','TS_term','list_id','TS_text','match_term_list','identifier_list','similarity_list','Judgement','Matched Count','Expected Count','ts_filename','fa_filename']]
            ...
            map_width = {colname2id['index']: 5,
                        ...
                        #  colname2id['TS_text_old']: 40,
                        #  colname2id['merge_reason']: 13,
                        ...
                        #  colname2id['New Judgement']: 20,
                        #  colname2id['New Matched Count']: 13,
                        #  colname2id['New Expected Count']: 13,
                        ...
                        #  colname2id['identical_identifier_count']: 8
                         }
        else:
        ...
        hidden_col_id = None
        if self.with_annot:
            hidden_col_id = [0, colname2id['index'], 
                             colname2id['text_block_id'],
                             colname2id['page_id'],
                             colname2id['phrase_id']]
        # else:
        #     hidden_col_id = [0, colname2id['TS_text_old'], 
        #                      colname2id['merge_reason']]
        if not self.with_annot:
            enlarge_col = ['TS_term', 'list_id', 'Judgement', 'Matched Count', 'Expected Count'] # , 'New Judgement', 'New Matched Count', 'New Expected Count', 'Old Judgement', 'Old Matched Count', 'Old Expected Count'
            enlarge_col_id = [colname2id[i] for i in enlarge_col]
            enlarge_col2 = ['index', 'text_block_id', 'page_id', 'phrase_id', 'TS_text', 'identifier_list', 'similarity_list'] # , 'TS_text_old', 'identifier_list_old', 'similarity_list_old'
        ...
        # if not self.with_annot:
        #     for row_num in self.df.index[~(self.df['New Matched Count'].isnull())].tolist():
        ...
        #     worksheet.filter_column(chr(65 + colname2id['identical_identifier_count']), "identical_identifier_count == 0")
...
if __name__=="__main__":
    ...
    parser.add_argument(
        "--term_match_folder",
        default='/home/data/ldrs_analytics/data/term_matching_csv/tm__v5.5_fa_v5.1.1_unannotated',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--output_folder",
        default='/home/data/ldrs_analytics/data/term_matching_csv/tm__v5.5_fa_v5.1.1_unannotated',  # f'/home/data2/ldrs_analytics/data/antd_ts_merge_fa/0304_lu/{result_tag}/(0)'
        type=str,
        required=False,
    ...
    output_fname = f'term_match_result_unannotated_all_round_3.xlsx'
