...
def col2dic(df, col_a, col_b):
    '''
    to generate a dictionary from 2 columns of a DataFrame, where values in col_a as keys, values in col_b as values
    @param df: pandas DataFrame that contains column names "col_a" and "col_b"
    @param col_a: column name in string that acts as key of dictionary
    @param col_b: column name in string that acts as value of dictionary
    @return col_a_to_col_b:
    '''
    col_a_to_col_b = pd.Series(df[col_b].values, index=df[col_a]).to_dict()
    return col_a_to_col_b
...
def main(df_ts, df_fa, topN=TOP_N_RESULTS, task_id=None, model_path=SENT_BERT_MODEL_PATH):
    ...
    id2sec_subsec = col2dic(df_fa, 'identifier', 'section_subsection')
            ...
            if match_type == 'content':
                if not df_ts_split_updated.empty:
                    ...
                    if not df_section_s.empty:
                        all_top_idf.extend(df_section_s.identifier_list.tolist()[0])
                        all_top_fa_text.extend(df_section_s.match_term_list.tolist()[0])
                        all_top_scores.extend(df_section_s.similarity_list.tolist()[0])
                        all_match_types.extend(df_section_s.match_type_list.tolist()[0])
